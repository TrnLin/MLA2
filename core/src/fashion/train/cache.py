...
def implementation_sha256_at_commit(
    *paths: str | Path,
    ...
    prefix = git_project_prefix(repository)
    manifest: list[dict[str, str]] = []
    for relative in sorted(relative_paths):
        # New commits nest sources under core; old receipts record the original tree.
        for tree_path in dict.fromkeys((prefix + relative, relative)):
            object_name = f"{commit}:{tree_path}"
