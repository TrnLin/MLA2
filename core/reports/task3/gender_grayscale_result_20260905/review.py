"""Verify the saved 04ac screen without importing PyTorch or running inference."""
...
from fashion.train.task3_gender_grayscale import NAME, PARENT_RUN_IDS
...
def main():
    ...
    groups = {name: {} for name in ("G2", "E6", "Drop30 + darkening", "Drop30 + darkening + gray")}
    ...
    for directory in sorted((SAVED / "comparison_ieee_v2").iterdir()):
        ...
        if run_id in original["run_ids"].values():
            name, source = "Drop30 + darkening + gray", SAVED / run_id
    ...
    for fold in (0, 4):
        child = groups["Drop30 + darkening + gray"][fold]
    ...
    child = groups["Drop30 + darkening + gray"]
    ...
    same(
        original["direct_parent_comparison"],
        {
            "name": "Drop30Dark",
            "classifier_dropout": 0.30,
            "training_augmentation": dataset_v2_spec(
                PREVIOUS_NAME, ORIGINAL_DROPOUT_RUN_IDS
    ...
    inspect_grayscale_changes(groups, classes)
    plot(summary, class_frame, robust)


def inspect_grayscale_changes(groups, classes):
    rows, switches, frames = [], [], {}
    for name, runs in groups.items():
        for condition in ("clean", "grayscale"):
            frame = (
                _pool(runs)
                if condition == "clean"
                else pd.concat(
            ...
            frames[name, condition] = frame
            for metric in oof_metrics(frame, classes)["per_class"]:
                rows.append(dict(model=name, condition=condition, **metric))
    parent, child = "Drop30 + darkening", "Drop30 + darkening + gray"
    for condition in ("clean", "grayscale"):
        left, right = frames[parent, condition], frames[child, condition]
        pd.testing.assert_frame_equal(
            left[["id", "cv_fold", "true_label", "product_family_group"]],
            right[["id", "cv_fold", "true_label", "product_family_group"]],
        )
        for label in classes:
            selected = left.true_label.eq(label)
            old = left.predicted_label.eq(left.true_label)
            new = right.predicted_label.eq(right.true_label)
            switches.append(
                dict(
                    condition=condition,
                    class_name=label,
                    support=int(selected.sum()),
                    parent_correct=int((selected & old).sum()),
                    candidate_correct=int((selected & new).sum()),
                    newly_wrong=int((selected & old & ~new).sum()),
                    recovered=int((selected & ~old & new).sum()),
                )
            )
    pd.DataFrame(rows).to_csv(HERE / "verified_clean_grayscale_classes.csv", index=False)
    pd.DataFrame(switches).to_csv(HERE / "verified_prediction_switches.csv", index=False)


def plot(summary, classes, robust):
    names = ["G2", "Drop30 + darkening", "Drop30 + darkening + gray"]
    ...
    axes[0, 0].set(
        ...
        ylim=(0, 1.11),
        ylabel="Macro-F1",
        title="Grayscale keeps clean validation close",
    )
    ...
    axes[1, 0].set(
        ...
        title="Raw grayscale scores improve",
    )
    axes[1, 0].legend(loc="lower left", fontsize=9)
    c = (
        classes.loc[classes.model.eq("Drop30 + darkening + gray")]
    ...
    fig.suptitle(
        "Gender occasional grayscale (10%): 18 of 19 rules pass\n"
