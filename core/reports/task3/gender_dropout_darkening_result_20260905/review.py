"""Verify the saved 04aa screen without importing PyTorch or running inference."""
...
def main():
    ...
    groups = {name: {} for name in ("G2", "E6", "Drop30", "Drop30 + darkening")}
    ...
    for directory in sorted((SAVED / "comparison_ieee_v2").iterdir()):
        ...
        if run_id in original["run_ids"].values():
            name, source = "Drop30 + darkening", SAVED / run_id
        elif run_id in PARENT_RUN_IDS:
            name, source = "Drop30", HERE / "parents" / run_id
        elif run_id.startswith("t3_gender_e6_gem_p3_"):
            name, source = "E6", HERE / "parents" / run_id
        else:
    ...
    for fold in (0, 4):
        child = groups["Drop30 + darkening"][fold]
        assert child["run_id"] == original["run_ids"][str(fold)]
        assert groups["Drop30"][fold]["run_id"] == spec.parent_run_id_for_fold(fold)
        _verify_training_evidence(
            child,
            spec,
        ...
        parent_spec = groups["Drop30"][fold]["config"]["child_experiment"]
        assert (
            parent_spec["name"] == "gender_dropout_030"
            and parent_spec["classifier_dropout"] == 0.30
        )
        assert parent_spec["training_augmentation"] == "translation_uniform_2px_p05"
        assert groups["Drop30"][fold]["config"]["parent_run_id"] == groups["G2"][fold]["run_id"]
        assert groups["G2"][fold]["config"]["parent_run_id"] == groups["E6"][fold]["run_id"]
        _verify_training_evidence(
            groups["Drop30"][fold],
            dataset_v2_spec("gender_dropout_030", reference_ids["G2"]),
            groups["G2"][fold]["run_id"],
            evidence,
        )
        same(
...
def plot(summary, classes, robust):
    names = ["G2", "Drop30", "Drop30 + darkening"]
    ...
    axes[0, 1].set(
        ...
        title="Mean gap is worse than dropout alone",
    )
    ...
    axes[1, 0].set(
        ...
        title="Large lighting gain; small grayscale gain",
    )
    axes[1, 0].legend(loc="lower left", fontsize=9)
    c = (
        classes.loc[classes.model.eq("Drop30 + darkening")]
    ...
    axes[1, 1].set(
        ...
        title="Girls and Unisex still have the largest gaps",
    )
    axes[1, 1].legend(loc="lower right", fontsize=9)
    fig.suptitle(
        "Gender dropout + mild darkening: screen failed\n"
