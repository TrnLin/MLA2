...
def test_balanced_weights_match_sklearn_formula_and_keep_absent_zero() -> None:
    result = build_task1_loss_weights(
        _training_rows(),
        {"a": 0, "b": 1, "c": 2, "d": 3},
        validation_fold=0,
        config=TASK1_BALANCED_WEIGHTED_LOSS,
    )

    assert result.class_counts == (1, 4, 9, 0)
    assert result.class_weights == pytest.approx((4.6666667, 1.1666667, 0.5185185, 0.0))
...
def test_balanced_weights_are_not_clipped() -> None:
    ...
    assert result.class_weights[0] == pytest.approx(9000.1)
    assert result.class_weights[1:] == pytest.approx((0.90001,) * 9)
