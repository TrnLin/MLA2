...
def test_classical_final_runs_two_selected_candidates_on_exactly_five_folds(
    ...
    assert all(
        set(group["fold"]) == set(range(5)) for _, group in fold_metrics.groupby("candidate_id")
    )
    ...
    for candidate_id in result.oof_predictions:
        per_class = pd.read_csv(evidence_root / f"per_class_classical_{candidate_id}.csv")
...
def test_classical_final_rejects_stale_selection_file(tmp_path: Path, changed_key: str) -> None:
