...
def test_mixup_keeps_canonical_rows_and_requires_source_evidence(project):
    ...
    spec = screen.mixup40_spec(project)
    ...
    with pytest.raises(ValueError, match="audit is required"):
        screen.require_mixup40_prerequisites(None, spec=spec, fold=0, root=project)
    with pytest.raises(ValueError, match="frozen label"):
        screen.training_splits(
            screen.MixUp40Spec(json.dumps({"labels_sha256": "wrong"})), root=project
        )
...
def test_complete_training_loop_saves_verifiable_unmixed_gap_and_mixup_receipt(
    ...
    monkeypatch.setattr(
        screen, "mixup40_config", lambda *a, **kw: Task3BaselineConfig(target="gender")
    )
    monkeypatch.setattr(
        screen,
        "require_mixup40_prerequisites",
        lambda *a, **kw: {
    ...
    screen.verify_mixup40_evidence(run, fold=0, splits=splits, directory=run_dir)
    ...
    assert run["config"]["mixup_contract"]["policy"]["alpha"] == 0.4
    metrics = run["metrics"]
...
def test_prerequisites_bind_mixup_code_rows_labels_and_direct_parent(project, monkeypatch):
    ...
    (project / "screen_decision.json").write_text('{"status":"pass"}')
    paths = {"precision_directory": str(project / "precision"), "mixup_directory": str(project)}
    ...
    monkeypatch.setattr(
        screen, "check_gender_stronger_mixup_sources", lambda **kw: (sources, [], spec, evidence)
    )
    monkeypatch.setattr(screen, "require_narrow_prerequisites", lambda *a, **kw: evidence)
    assert (
        screen.require_mixup40_prerequisites(audit, spec=spec, fold=0, root=project)[
            "parent_directory"
        ]
        == project / "parent-0"
    )
    with pytest.raises(ValueError, match="verified 04af parent"):
        screen.require_mixup40_prerequisites(
