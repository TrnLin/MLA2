"""Evaluate the fixed Usage E8 refit against its saved five-model holdout average."""
...
from fashion.train.task3_usage_e8_refit import CLASSES, ROW_COLUMNS, prepare_refit

REPORT = Path(__file__).resolve().parent
REFIT = ROOT / "results/evidence/task3/usage_e8_refit_20260907"
...
def verify_inputs():
    ...
    assert payload["base_config"] == config.to_dict()
    ...
    assert checkpoint["class_weight_contract"] == payload["class_weight_contract"]
    assert read(REFIT / "class_weights.json") == payload["class_weight_contract"]
    class_map = read(REFIT / "class_map.json")
    assert class_map["classes"] == CLASSES
    assert class_map["label_to_index"] == {name: i for i, name in enumerate(CLASSES)}
    ...
    names = ["holdout_image_manifest.csv", "E8/usage_holdout_probabilities.csv"] + [
        f"E8/usage_holdout_fold_{fold}.csv" for fold in range(5)
    ]
    for name in names:
    ...
    average = pd.read_csv(AVERAGE / "E8/usage_holdout_probabilities.csv", keep_default_na=False)
    assert average.id.tolist() == holdout.id.tolist()
    folded = []
    for fold in range(5):
        frame = pd.read_csv(AVERAGE / f"E8/usage_holdout_fold_{fold}.csv", keep_default_na=False)
...
def main():
    ...
    save(
        {
            ...
            "user_authorization": (
                ...
                "to the average one; follow-up review of Usage E8 + translation refit"
            ),
    ...
    for name, values in [
        ...
        result["macro_f1_classes_present"] = float(
            np.mean([item["f1"] for item in result["per_class"] if item["support"] > 0])
        )
    ...
    for key in ("accuracy", "macro_f1", "balanced_accuracy", "nll", "ece_15"):
        np.testing.assert_allclose(
            metrics["Five-model average"][key], prior_metrics["models"]["E8"][key]
        )
    ...
    axes[0].set(
        title="E8 refit training completed",
        xlabel="Epoch",
        ylabel="Weighted training loss",
        xlim=(1, 30),
    )
    scores = (
    ...
    figure_path = ROOT / "results/figures/task3/usage_e8_refit_holdout_20260907.png"
