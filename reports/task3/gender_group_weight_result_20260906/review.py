"""Reproduce the 04ae decision, frozen weights and direct-parent error changes."""
...
def main():
    ...
    contract, spec = label_contract(ROOT), group_weight_spec(ROOT)
    ...
    assert audit["weight_implementation_sha256"] == compute_sha256(
        ROOT / "src/fashion/train/task3_gender_group_weight.py"
    )
    recorded_implementation = subprocess.check_output(
        ["git", "show", f"{COMMIT}:src/fashion/train/task3_gender_group_weight.py"], cwd=ROOT
    )
    assert (
        hashlib.sha256(recorded_implementation).hexdigest() == audit["weight_implementation_sha256"]
    )
    for fold in (0, 4):
        train = get_samples(get_cv_split(splits, fold)[0], target="gender")
        weighted, weight_contract = add_gender_article_weights(train, validation_fold=fold)
        same(audit["training_weight_contracts"][str(fold)], weight_contract)
        pd.testing.assert_frame_equal(train, weighted[train.columns])
        assert np.allclose(
            weighted.groupby("articleType").gender_article_weight.sum(),
            train.groupby("articleType").size(),
        )
    assert (SAVED / "training_weight_preview.csv").read_text() == preview_gender_article_weights(
        splits
    ).to_csv(index=False)
    ...
    groups = {name: {} for name in ("G2", "E6", "Gray10", "NameTruth", "GroupWeight")}
    ...
    for directory in sorted((SAVED / "comparison_name_truth_ieee").iterdir()):
        ...
        if run_id in saved["run_ids"].values():
            name, source = "GroupWeight", SAVED / run_id
        ...
        run = inspect_gender_run(
            source,
            registry=registry,
            splits=splits if name in {"NameTruth", "GroupWeight"} else canonical,
            classes=classes,
            root=ROOT,
        )
        fold = run["fold"]
        ...
        if name == "GroupWeight":
            verify_weight_evidence(run, fold=fold, splits=splits, directory=source)
            _verify_training_evidence(
                run,
                spec,
    ...
    teacher_interval = paired_family_bootstrap(
        pooled_frames["GroupWeight", "original_teacher"],
    ...
    changes = []
    old, new = pooled_frames["NameTruth", "name_truth"], pooled_frames["GroupWeight", "name_truth"]
...
def analyze_errors(old, new, frames, classes, splits, canonical):
    ...
    joined["weighted_prediction"] = new.predicted_label.to_numpy()
    joined["confidence"] = new.confidence.to_numpy()
    joined["parent_correct"] = old.predicted_label.eq(old.true_label).to_numpy()
    joined["weighted_correct"] = new.predicted_label.eq(new.true_label).to_numpy()
    joined["recovered"] = ~joined.parent_correct & joined.weighted_correct
    joined["newly_wrong"] = joined.parent_correct & ~joined.weighted_correct
    joined["persistent_error"] = ~joined.parent_correct & ~joined.weighted_correct
    ...
    groups["net_correct"] = groups.weighted_correct - groups.parent_correct
    groups["parent_errors"] = groups.rows - groups.parent_correct
    groups["weighted_errors"] = groups.rows - groups.weighted_correct
    ...
    for model, column in (
        ...
        ("GroupWeight", "weighted_prediction"),
    ):
    ...
    detail = {
        ...
        "weighted_errors": int((~joined.weighted_correct).sum()),
...
def plot_examples(rows):
    ...
    for ax, row in zip(axes.flat, cases):
        ...
        ax.set_title(
            ...
            f"{row.parent_prediction} → {row.weighted_prediction}",
            fontsize=9,
        )
    fig.suptitle(
        "Selected changes after weighting — one high-confidence example per class and direction",
        fontsize=13,
    )
...
def plot(summary, classes, robust):
    ...
    pair = ("NameTruth", "GroupWeight")
    labels = ("Name-label parent", "Group weighted")
    colors = ("#8c9aa5", "#168879")
    selected = classes[classes.basis.eq("name_truth")]
    class_names = ["Boys", "Girls", "Men", "Unisex", "Women"]
    for ax, metric, title in (
        (axes[0, 0], "f1", "Class F1 — small minority gains, adult losses"),
    ...
    ax.set(ylim=(0.65, 1.04), title="Gap shrinks a little; training fit stays high")
    ...
    ax.set(
        ...
        ylim=(0, 1),
        title="Mean corrupted F1 — darkening is slightly worse",
    )
    ax.legend(loc="lower left", fontsize=9)
    fig.suptitle(
        "04ae: one weighting trial on the same name-truth labels and folds 0 + 4", fontsize=15
    )
